...
**Worked case study.** [`examples/case_study_pet/`](examples/case_study_pet/) applies four readings to one anonymized pair of real chromatograms (a virgin and a recycled PET, single-quadrupole GC-MS): `run_case_study.py` regenerates [`report.html`](examples/case_study_pet/report.html) and its GitHub-rendered companion [`report.md`](examples/case_study_pet/report.md) (same numbers, committed dual-theme SVG figures) from the shipped data — letters, events, and grammar side by side on the same two signals, then a fourth panel that reconstructs pseudo-EI spectra from co-eluting differential events (blind-first, raw amplitudes, base peak = 100) and reads them along the two faces of recycling — degradation/history vs contamination/cleanliness — with an excess-support gate that names apex/class shifts as such and an honest category for chemistry beyond the frozen marker dictionaries.
