...
# Maintained by Generative Simulation\\olivier.vitrac@gmail.fr
# Revision history: 2025-05-24
...
if __name__ == "__main__":
    ...
    if not os.path.basename(script_dir) == "utils":
        print("Error: This script must be run from the repo/utils/ directory.")
        sys.exit(1)

    # Adjust the paths
    GSpackageroot = "sig2dna_core"
    mainfolder = os.path.abspath(os.path.join(script_dir, "../"))
    GSrepo_base_path = os.path.join(mainfolder, GSpackageroot)  # Base path for GSrepo package
    ...
    sys.path.insert(0, GSrepo_base_path)  # Add GSrepo directory as package root

    # Packages to process
    packages = [GSpackageroot]

    # Generate __all__ for each package
    for package in packages:
        generate_all_for_package(package, GSrepo_base_path, GSpackageroot, all_dir)
