...
# for all modules found in $mainfolder/$GSpackage
#
# Prerequisites:
#  - Python 3.x
...
# Check if the '../$GSpackage' directory exists
GSpackage="sig2dna_core"
if [[ ! -d "../$GSpackage" ]]; then
    echo "Error: '../$GSpackage' directory not found. Adjust mainfolder path accordingly."
    exit 1
fi
...
mainfolder="$(realpath ../)"
output_markdown="$mainfolder/docs/html/${GSpackage}_classes_documentation.md"
...
find_cmd="find \"$mainfolder/$GSpackage\" -type f -name \"*.py\""
