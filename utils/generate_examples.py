...
**Usage:**
---------
Run the script from the `{GSrepo}/utils/` directory using the following command:
...
```bash
mainfolder="$(realpath ../)"
find "$mainfolder/$GSpackage" "$mainfolder/$GSpackage/private" -maxdepth 1 -type f -name '*.py' | sed "s|$mainfolder/||" | sort > ../html/modules_withexamples_list.txt
```

For production use this minimal example (see generate_diagrams.sh)
```bash
    mainfolder="$(realpath ../)"
    output_markdown="$mainfolder/html/patanakar_classes_documentation.md"
    ...
    lookfolders=(
        "$mainfolder/$GSpackage"
        "$mainfolder/$GSpackage/private"
    )
...
**Inputs:**
---------
- **Standard Input (`STDIN`):** A list of module paths in the format `$GSpackage.module_name.py` (e.g., `$GSpackage.migration.py`).
...
def ensure_run_from_utils():
    """Ensure the script is run from /utils/ directory by checking for pdocme.sh."""
    if not os.path.isfile("generate_diagrams.sh"):
        print(f"Error: This script must be run from the {GSrepo}/utils/ directory.", file=sys.stderr)
        sys.exit(1)

def get_version():
    """Extract the version number of {GSrepo} from version_file."""
...
def organize_modules(module_paths):
    ...
    module_paths: List of module paths relative to mainfolder, e.g., '$GSpackage/geometry.py'
    Returns a nested dictionary representing the folder structure.
    """
    tree = {}
    for path in module_paths:
        parts = Path(path).parts  # e.g., ('$GSpackage', 'migration.py') or ('$GSpackage', 'private', 'geometry.py')
        current = tree
...
<p>Back to the <a href='index.html'>Python'{GSrepo} documentation</a>.</p>
...
<title>{GSrepo} Usage/Class Examples</title>
<style>
{base_css}
...
<header>
    ...
    <h1>{GSrepo} Documentation - Usage Examples</h1>
</header>
<div id='content'>
<div id='nav'>
<p><strong>Version:</strong> {GSrepo} v.{get_version()}</p>
...
<footer>
<p>&copy; {datetime.now().year} {GSrepo} Project. All rights reserved.</p>
</footer>
<script>
